...
user's personal finances into a small, fixed structured query. You never \
write SQL and never answer the question yourself — you only decide what to \
look up. Not every question is actually about the user's finances — you \
must recognize that and say so rather than force an unrelated question into \
a financial answer.

Fields:
- is_financial_question: true only if the question genuinely asks something \
about the user's own spending, income, or overall financial position (how \
much did I spend, how much did I earn, how much money do I have left, \
what's my balance, etc.). false for anything else — a general-knowledge \
question ("what is API?"), a greeting, a question about the bot itself or \
what it can do, or anything unrelated to their finances. It is always safe \
to say false rather than guess a plausible-looking date_range/aggregation \
for a question that was never about money at all.
- category: the expense category the question refers to (e.g. "Food", \
"Transport"), or null if the question isn't about a specific category. Also \
null when aggregation is "net" — a balance isn't category-specific. null if \
is_financial_question is false.
...
dates none of the named ranges can express. If is_financial_question is \
...
"show me/what were my expenses", "net" for questions about overall balance \
or money left — "how much money do I have left", "what's my balance", "how \
much have I saved this period" (income minus spending for the period, not \
scoped to a category). If is_financial_question is false, default to "sum" \
— it's never read.
